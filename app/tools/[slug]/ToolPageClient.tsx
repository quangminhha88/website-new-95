...
/**
 * Client portion of /tools/[slug]. Receives pre-fetched data from the
 * Server Component (../page.tsx) and renders the same JSX that used to
 * live in src/views/tools/ToolPage.tsx.
 *
 * Six changes from the source view:
 *   1. 'use client' directive added (above)
 *   2. Accepts props {tool, category, relatedTools, schema,
 *      indexability, breadcrumbs} instead of fetching
 *   3. Removed useTool, useCategoryById, useRelatedTools, useParams
 *   4. useDynamicCTA + useTrackPage kept identical
 *   5. Link comes from next/link (already done at view level in Stage 4)
 *   6. All className, icons, layout untouched
 *
 * The two helper components (LoadingSpinner, ErrorState) and the local
 * derivations of schema/indexability/breadcrumbs/faqs are gone — they
 * either never run (data is guaranteed by the server) or have moved
...
import type { IndexabilityVerdict } from '@/seo/indexability';

interface ToolPageClientProps {
  tool: Tool;
  category: Category | null;
  /** Narrow projection — matches getRelatedToolsServer's columns. */
  relatedTools: Array<Pick<Tool, 'id' | 'name' | 'slug' | 'logo_url' | 'tagline' | 'category_id'>>;
  schema: object | null;
